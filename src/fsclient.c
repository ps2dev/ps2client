...
int main(int argc, char **argv, char **env)
{
    ...
    for (loop0 = 0; env[loop0]; loop0++) {
        ...
        if (strncmp(env[loop0], "PS2HOSTNAME", 11) == 0) {
            strncpy(hostname, &env[loop0][12], sizeof(hostname));
        }
    }
    ...
    for (loop0 = 1; argv[loop0]; loop0++) {
        ...
        if (strncmp(argv[loop0], "-h", 2) == 0) {
            loop0++;

            // Check to make sure the hostname value was actually supplied.
            if (argc == loop0) {
                printf("Error: No hostname was supplied the '-h' option.\n");
                print_usage();
                return -1;
            }
    ...
    if (argc < 0) {
        printf("Error: No command was supplied.\n");
        print_usage();
        return -1;
    }
    ...
    if (ps2netfs_connect(hostname) < 0) {
        printf("Error: Could not connect to the ps2netfs server. (%s)\n", hostname);
        return -1;
    }

    // Perform the requested command.
    if (strcmp(argv[-1], "copyfrom") == 0) {
        return fsclient_copyfrom(argv[0], argv[1]);
    } else if (strcmp(argv[-1], "copyto") == 0) {
        return fsclient_copyto(argv[0], argv[1]);
    } else if (strcmp(argv[-1], "delete") == 0) {
        return fsclient_delete(argv[0]);
    } else if (strcmp(argv[-1], "devlist") == 0) {
        return fsclient_devlist();
    } else if (strcmp(argv[-1], "dir") == 0) {
        return fsclient_dir(argv[0]);
    } else if (strcmp(argv[-1], "mkdir") == 0) {
        return fsclient_mkdir(argv[0]);
    } else if (strcmp(argv[-1], "mount") == 0) {
        return fsclient_mount(argv[0], argv[1]);
    } else if (strcmp(argv[-1], "rmdir") == 0) {
        return fsclient_rmdir(argv[0]);
    } else if (strcmp(argv[-1], "sync") == 0) {
        return fsclient_sync(argv[0]);
    } else if (strcmp(argv[-1], "umount") == 0) {
        return fsclient_umount(argv[0]);
    } else
    ...
    if (ps2netfs_disconnect() < 0) {
        printf("Error: Could not disconnect from the ps2netfs server. (%s)\n", hostname);
        return -1;
    }
...
int fsclient_copyfrom(char *source, char *destination)
{
    ...
    size = ps2netfs_command_lseek(fd0, 0, LSEEK_END);
    ps2netfs_command_lseek(fd0, 0, LSEEK_SET);
    if (size < 0) {
        printf("Error: Get source file size failed. (%d)\n", size);
        return -1;
    }
...
// Open the destination file.
...
#endif
    if (fd1 < 1) {
        printf("Error: Open destination file failed. (%d)\n", fd1);
        return -1;
    }
    ...
    while (total < size) {
        ...
        if (result < 0) {
            printf("Error: Read source data failed. (%d)\n", result);
            return -1;
        }
        ...
        if (result < 0) {
            printf("Error: Write destination data failed. (%d)\n", result);
            return -1;
        }
    ...
    if (result < 0) {
        printf("Error: Close source file failed. (%d)\n", result);
        return -1;
    }
    ...
    result = close(fd1);
    if (result < 0) {
        printf("Error: Close destination file failed. (%d)\n", result);
        return -1;
    }
...
int fsclient_delete(char *pathname)
{
    ...
    if (result < 0) {
        printf("Error: Delete file failed. (%d)\n", result);
        return -1;
    }
...
int fsclient_devlist(void)
{
    ...
    if (devcount < 0) {
        printf("Error: Get device listing failed. (%d)\n", result);
        return -1;
    }
    ...
    for (loop0 = 0; loop0 < devcount; loop0++) {
        ...
        if (!strcmp(temp, "rom")) {
            printf("  rom    - Onboard rom device.\n");
        } else if (!strcmp(temp, "cdrom")) {
            printf("  cdrom  - Standard cd/dvd device. (cdrom:)\n");
        } else if (!strcmp(temp, "host")) {
            printf("  host   - Host file system. (host:)\n");
        } else if (!strcmp(temp, "mc")) {
            printf("  mc     - Memory card driver. (mc0: mc1:)\n");
        } else if (!strcmp(temp, "hdd")) {
            printf("  hdd    - Internal HDD unit.\n");
        } else if (!strcmp(temp, "pfs")) {
            printf("  pfs    - Playstation File System.\n");
        } else if (!strcmp(temp, "dev9x")) {
            printf("  dev9x  - Blah blah blah.\n");
        } else
...
int fsclient_dir(char *pathname)
{
    ...
    if (dd < 0) {
        printf("Error: Open directory failed. (%d)\n", dd);
        return -1;
    }
    ...
    while (ps2netfs_command_dread(dd, &dirent) > 0) {
        printf("  ");

        // Output the mode information.
        if (ntohl(dirent.mode) & 0x4000) {
            printf("l");
        } else if (ntohl(dirent.mode) & 0x2000) {
            printf("-");
        } else if (ntohl(dirent.mode) & 0x1000) {
            printf("d");
        } else {
            printf("-");
        }
        if (ntohl(dirent.mode) & 0x0100) {
            printf("r");
        } else {
            printf("-");
        }
        if (ntohl(dirent.mode) & 0x0080) {
            printf("w");
        } else {
            printf("-");
        }
        if (ntohl(dirent.mode) & 0x0040) {
            printf("x");
        } else {
            printf("-");
        }
        if (ntohl(dirent.mode) & 0x0020) {
            printf("r");
        } else {
            printf("-");
        }
        if (ntohl(dirent.mode) & 0x0010) {
            printf("w");
        } else {
            printf("-");
        }
        if (ntohl(dirent.mode) & 0x0008) {
            printf("x");
        } else {
            printf("-");
        }
        if (ntohl(dirent.mode) & 0x0004) {
            printf("r");
        } else {
            printf("-");
        }
        if (ntohl(dirent.mode) & 0x0002) {
            printf("w");
        } else {
            printf("-");
        }
        if (ntohl(dirent.mode) & 0x0001) {
            printf("x");
        } else {
            printf("-");
        }
    ...
    if (result < 0) {
        printf("Error: Close directory failed. (%d)\n", result);
        return -1;
    }
...
int fsclient_mkdir(char *pathname)
{
    ...
    if (result < 0) {
        printf("Error: Make directory failed. (%d)\n", result);
        return -1;
    }
...
int fsclient_mount(char *device, char *fsname)
{
    ...
    if (result < 0) {
        printf("Error: Mount device failed. (%d)\n", result);
        return -1;
    }
...
int fsclient_rmdir(char *pathname)
{
    ...
    if (result < 0) {
        printf("Error: Remove directory failed. (%d)\n", result);
        return -1;
    }
...
int fsclient_sync(char *device)
{
    ...
    if (result < 0) {
        printf("Error: Sync device failed. (%d)\n", result);
        return -1;
    }
...
int fsclient_umount(char *device)
{
    ...
    if (result < 0) {
        printf("Error: Umount device failed. (%d)\n", result);
        return -1;
    }
