...
int ps2netfs_connect(char *hostname)
{
    ...
    if ((ps2netfs_socket = network_connect(hostname, 0x4713, SOCK_STREAM)) < 0) {
        return -1;
    }
...
int ps2netfs_disconnect(void)
{
    ...
    if (network_disconnect(ps2netfs_socket) < 0) {
        return -1;
    }
...
int ps2netfs_command_open(char *pathname, int flags)
{
    ...
    if (pathname) {
        strcpy(command.pathname, pathname);
    }

    // Send the command packet.
    if (network_send(ps2netfs_socket, &command, sizeof(command)) < 0) {
        return -1;
    }

    // Receive the response packet.
    if (network_receive_all(ps2netfs_socket, &response, sizeof(response)) < 0) {
        return -1;
    }
...
int ps2netfs_command_read(int fd, void *buffer, int size)
{
    ...
    if (network_receive_all(ps2netfs_socket, buffer, ntohl(response.result)) < 0) {
        return -1;
    }
...
int ps2netfs_command_write(int fd, void *buffer, int size)
{
    ...
    if (network_send(ps2netfs_socket, buffer, size) < 0) {
        return -1;
    }
...
int ps2netfs_command_sync(char *device, int flags)
{
    ...
    command.flags = htonl(flags);
    if (device) {
        strncpy(command.device, device, 256);
    }
...
int ps2netfs_command_mount(char *device, char *fsname, int flags, char *argv, int argc)
{
    ...
    if (fsname) {
        strncpy(command.fsname, fsname, 256);
    }
    if (argv) {
        strncpy(command.argv, argv, 256);
    }
